...
public class ExpectationHorizonRepository {
	...
	public EhCategory saveCategory(final EhCategory category) {
		...
		jdbc.update("""
				...
				.addValue("descriptionMarkdown", category.descriptionMarkdown()).addValue("sortOrder", category.sortOrder()));
		return category;
	}
	...
	public void moveNoteSection(final ExamNoteSection noteSection, final int offset) {
		...
		move("exam_note_section", siblings.stream().map(SortableItem::fromNoteSection).toList(), noteSection.id(), offset);
	}
	...
	private EhPart insertPart(final EhPart part) {
		...
		jdbc.update("""
				...
				.addValue("sortOrder", part.sortOrder()), keyHolder, new String[] { "id" });
	...
	private EhCategory insertCategory(final EhCategory category) {
		...
		jdbc.update("""
				...
				.addValue("sortOrder", category.sortOrder()), keyHolder, new String[] { "id" });
	...
	private EhTask insertTask(final EhTask task) {
		...
		jdbc.update("""
				...
				""", new MapSqlParameterSource().addValue("categoryId", task.categoryId()).addValue("title", task.title())
				.addValue("sortOrder", task.sortOrder()), keyHolder, new String[] { "id" });
	...
	private EhRequirement insertRequirement(final EhRequirement requirement) {
		...
		jdbc.update("""
				...
				.addValue("descriptionMarkdown", requirement.descriptionMarkdown())
				.addValue("maxPoints", requirement.maxPoints()).addValue("bonus", requirement.bonus())
				.addValue("sortOrder", requirement.sortOrder()), keyHolder, new String[] { "id" });
		return new EhRequirement(generatedId(keyHolder, "EH requirement"), requirement.taskId(),
				requirement.descriptionMarkdown(), requirement.maxPoints(), requirement.bonus(), requirement.sortOrder());
	}
	...
	private ExamNoteSection insertNoteSection(final ExamNoteSection noteSection) {
		...
		jdbc.update("""
				...
				""", new MapSqlParameterSource().addValue("examId", noteSection.examId()).addValue("title", noteSection.title())
				.addValue("descriptionMarkdown", noteSection.descriptionMarkdown())
				.addValue("sortOrder", noteSection.sortOrder()), keyHolder, new String[] { "id" });
	...
	private void move(final String table, final List<SortableItem> siblings, final Integer id, final int offset) {
		...
		jdbc.update("""
				...
				""".formatted(table),
				new MapSqlParameterSource().addValue("currentId", current.id())
						.addValue("targetId", target.id())

...
class ExpectationHorizonRepositoryTests {
	...
	void savesAndFindsExpectationHorizonHierarchy() {
		...
		final EhPart part = expectationHorizonRepository.savePart(
				new EhPart(null, exam.id(), "Klausurteil A", expectationHorizonRepository.nextPartSortOrder(exam.id())));
		...
		assertThat(expectationHorizonRepository.findRequirementsByExamId(exam.id())).containsExactly(updatedRequirement);
	}
	...
	void cascadesDeletedParents() {
		...
		final EhTask task = expectationHorizonRepository.saveTask(new EhTask(null, category.id(), "Teilaufgabe 1", 0));
		expectationHorizonRepository.saveRequirement(new EhRequirement(null, task.id(), "Formuliert sauber.", 4, false, 0));
	...
	void savesMovesAndDeletesNoteSections() {
		...
		final ExamNoteSection first = expectationHorizonRepository.saveNoteSection(
				new ExamNoteSection(null, exam.id(), "Hinweise", "Lies genau.", 0));
		final ExamNoteSection second = expectationHorizonRepository.saveNoteSection(
				new ExamNoteSection(null, exam.id(), "Notenschluessel", "15 Punkte = 1", 1));
		...
		assertThat(expectationHorizonRepository.findNoteSectionsByExamId(exam.id())).containsExactly(
				new ExamNoteSection(second.id(), exam.id(), "Notenschluessel", "15 Punkte = 1", 0));
	}

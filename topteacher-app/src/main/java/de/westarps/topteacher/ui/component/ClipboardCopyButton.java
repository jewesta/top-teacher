...
public class ClipboardCopyButton extends Composite<Button> {
	...
	private void configureButton(final String label, final String successMessage, final String failureMessage) {
		...
		/*
		 * These are custom DOM events dispatched by the browser-side clipboard
		 * handler. Vaadin has no typed component event for them, so we listen
		 * at element level to bridge the async result back into server-side
		 * notifications.
		 */
	...
	private void installClipboardListener() {
		/*
		 * Clipboard writes must run directly inside the browser click handler;
		 * a server round trip can lose the user activation and make the browser
		 * deny navigator.clipboard without prompting.
		 */

...
class CourseRosterWriterTests {
	...
	@Test
	void createsNewPupilsReactivatesReusedPupilsAndAssignsEachIdentityOnce() {
		...
		final Pupil inactive = new Pupil(42, "Anna", "Müller", Lifecycle.INACTIVE);
		final Pupil reactivated = new Pupil(42, "Anna", "Müller", Lifecycle.ACTIVE);
		...
		when(pupils.findById(42)).thenReturn(Optional.of(inactive));
		when(pupils.save(reactivated)).thenReturn(reactivated);
		...
		assertThat(result.pupils()).containsExactly(reactivated, createdPupil);
		verify(pupils).save(reactivated);
